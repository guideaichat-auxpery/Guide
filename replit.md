...
- **Streamlit Rerun Architecture**: **Critical UI Placement Pattern** - Quick prompt/guide cards in chat interfaces (e.g., Montessori Companion's 15 topic buttons) must render BEFORE chat history loops to prevent mid-conversation insertion during Streamlit reruns. **Implementation**: Cards render at top of interface (after headers/selectors), followed by separator line, then complete chat history loop, then new message generation. This ensures cards remain anchored at top and chat messages flow chronologically below. **Fixed November 2025**: Montessori Companion Quick Guides moved from line 507 to line 449 in `show_companion_interface()` (interfaces.py) to prevent cards from appearing between messages during multi-turn conversations.
...
- **Core Features**:
  ...
  - **Document Upload & Analysis**: 
    - **Montessori Companion**: Educators can upload teaching materials (TXT, PDF, DOCX, JPG, PNG) for Montessori-focused AI feedback. Upload section appears between Quick Guide cards and chat input (interfaces.py lines 565-631). System extracts text using PyPDF2, python-docx, and pytesseract OCR, then AI provides feedback through a Montessori lens. **PDF Safety**: Handles scanned/textless PDFs gracefully by replacing None values with empty strings and providing user-friendly fallback messages when text extraction fails.
